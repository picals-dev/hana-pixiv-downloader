...
> 版本：v0.6.0-draft
> 最后更新：2026-06-24
> 状态：net 层 SSOT、Pixiv 领域模块、失败回放闭环、四层测试架构与 ugoira GIF 下载能力均已落地并通过验证
...
- 当前主链路可概括为：
  ...
  4. crawler 拉取作品 ID，并先读取作品 detail 生成 `ArtworkDownloadPlan`
  5. 静态图作品走 `pages -> original image urls`；ugoira 作品走 `ugoira_meta.originalSrc zip -> GIF`
  6. downloader 流式写 `.part` 或 `.picals-workspace` 中间态，成功后原子落最终产物，失败形成 `FailureRecord`
  7. `tags.json` 导出复用规划阶段的 detail cache，不重复请求同一作品 detail
...
- 分阶段结论已经很明确：
  ...
  - 测试架构升级：已完成
  - ugoira GIF 正式集成：已完成
  - 当前下一阶段重点：README、安装分发、发布准备、体验打磨，以及后续是否需要更高画质策略评估
