...
links: ["picals-crawler-产品设计文档.md", "picals-crawler-项目理解基线.md", "typescript-原项目实现观察.md", "picals-crawler-测试架构升级实现记录-2026-06-23.md", "picals-crawler-技术设计附录-实现细节.md", "picals-crawler-ux-优化执行记录-2026-06-21.md", "picals-crawler-测试指南.md"]
...
> 版本：v0.5.0-draft
> 最后更新：2026-06-23
> 状态：net 层 SSOT、Pixiv 领域模块、失败回放闭环与四层测试架构均已落地并通过验证
...
- 当前主链路可概括为：
  ...
  3. 命令层创建共享 `PixivNetSession`
  4. crawler 拉取作品 ID / 图片 URL / tags 元数据
  5. downloader 流式写 `.part`，成功 rename，失败形成 `FailureRecord`
  6. 收尾阶段执行 auto-replay，并在仍失败时写 manifest 供 `retry` 回放
...
- **contracts**：selector fixture contract、manifest roundtrip 等公开契约。
- **app integration**：Mock HTTP server（`wiremock`）+ 临时目录，覆盖完整 `UserCrawler::run()`、断点续传、`retry` manifest 回放、auto-replay / session identity 与副作用。
- **cli acceptance**：`assert_cmd::Command::cargo_bin("picals-crawler")` + 临时 `HOME / XDG_CONFIG_HOME` + `wiremock`，覆盖 help、参数错误、`download user --dry-run`、`retry <manifest>` 与至少一条真实落盘断言。
...
- 分阶段结论已经很明确：
  ...
  - Phase 2：已完成
  - 测试架构升级：已完成
  - 当前下一阶段重点：README、安装分发、发布准备与体验打磨
